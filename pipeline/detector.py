...
class YOLODetectONNX:
    ...
    def predict_batch(self, images):
        ...
        # 预处理每张图, 收集 letterbox 参数
        tensors, ratios, pads, shapes = [], [], [], []
        for img in images:
            t, (rw, rh), (pw, ph) = self.preprocess(img)
            tensors.append(t)
            ratios.append((rw, rh))
            pads.append((pw, ph))
            shapes.append(img.shape[:2])

        batch = np.concatenate(tensors, axis=0)  # (B, 3, H, W)
        outputs = self.session.run(None, {self.input_name: batch})
        preds = outputs[0]  # (B, N, 6) or (1, B*N, 6)

        # Handle both output formats
        results = []
        if preds.shape[0] == len(images):
            # Per-image output: (B, N, 6)
            for i, (h0, w0) in enumerate(shapes):
                results.append(self._postprocess(
                    preds[i], (h0, w0), ratios[i], pads[i]))
        else:
            # Fallback: single output, run serial
...
class YOLOPtDetector:
    ...
    def predict(self, img_bgr):
        ...
        return self._parse_result(self.model(
            img_bgr, conf=self.conf, imgsz=self.imgsz,
            device=self.device, verbose=False)[0])

    def predict_batch(self, images):
        """Batch YOLO: ultralytics natively supports list-of-images."""
        if len(images) == 0:
            return []
        if len(images) == 1:
            return [self.predict(images[0])]
        results = self.model(images, conf=self.conf, imgsz=self.imgsz,
                             device=self.device, verbose=False)
        return [self._parse_result(r) for r in results]

    def _parse_result(self, r):
        """将 ultralytics Results 对象转为 list[dict]."""
        dets = []
        if r.boxes is None:
            return dets
